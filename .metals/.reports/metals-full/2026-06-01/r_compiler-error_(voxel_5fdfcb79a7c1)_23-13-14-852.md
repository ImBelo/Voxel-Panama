...
object Window {
  ...
  def start(width: Int, height: Int, title: String): Unit = {
    ...
    withArena(Confined) { arena =>
      ...
      val window = new GlfwWindow(height,widht,glfw, "Meow", arena)
      ...
      while (!shouldClose) {
        ...
        EngineProfiler.zone("Total Frame") {
          ...
          EngineProfiler.zone("Math & Uniform Updates") {
            ...
            // 2. Update Camera Transformations (Ensure these use pre-allocated fields inside Camera!)
            camera.getViewMatrix(viewMatrix)
            camera.getProjectionMatrix(projectionMatrix, window.currentAspectRatio) 
            modelMatrix.identity() 

            // 3. Set environmental lighting & color uniforms
            gl.uniform3f(colorLoc, 0.2f, 0.8f, 0.2f)      
            gl.uniform3f(lightPosLoc, 5.0f, 10.0f, 5.0f) 
            gl.uniform3f(lightColLoc, 1.0f, 1.0f, 0.95f) 

            // 4. Upload (Ensure setMatrix4f uses a pre-allocated native transfer buffer!)
