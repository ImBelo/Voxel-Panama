...
class Mesh(
  ...
  // 1. BIND THE VBO FIRST! (This prevents the SIGSEGV)
  gl.bindBuffer(GL_ARRAY_BUFFER, vboId)
  // Upload vertex data to the VBO...
  gl.bufferData(GL_ARRAY_BUFFER, /* vertex byte size */, /* vertex pointer */, GL_STATIC_DRAW)

  // 2. BIND THE EBO
  gl.bindBuffer(GL_ELEMENT_ARRAY_BUFFER, eboId)
  // Upload index data to the EBO...
  gl.bufferData(GL_ELEMENT_ARRAY_BUFFER, /* index byte size */, /* index pointer */, GL_STATIC_DRAW)

  // 3. NOW CONFIGURE THE POINTERS (OpenGL links these to the VBO we just bound above)
  val stride = 6 * java.lang.Float.BYTES
  val posOffset = 0L // Address 0 from the start of the VBO

  // Position Attribute
  gl.vertexAttribPointer(0, 3, GL_FLOAT, 0.toByte, stride, posOffset)
  gl.enableVertexAttribArray(0)

  // Normal Attribute
  val normalOffset = 3L * java.lang.Float.BYTES // Address 12 bytes from the start of the VBO
  gl.vertexAttribPointer(1, 3, GL_FLOAT, 0.toByte, stride, normalOffset)
  gl.enableVertexAttribArray(1)

  // 4. UNBIND VAO to protect this state
