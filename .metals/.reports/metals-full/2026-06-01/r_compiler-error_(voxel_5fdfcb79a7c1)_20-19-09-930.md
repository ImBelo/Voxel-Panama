...
object Window {
  ...
  def start(width: Int, height: Int, title: String): Unit = {
    ...
    withArena(Confined) { arena =>
      ...
      val projLoc   = shader.getUniformLocation("u_Projection")
      val viewLoc   = shader.getUniformLocation("u_View")
      val colorLoc  = gl.getUniformLocation(shader.programId, arena.allocateFrom("u_ObjectColor"))
      ...
      while (!shouldClose) {
        ...
        shader.use()
        shader.setMatrix4f(projLoc, projectionMatrix)
        shader.setMatrix4f(viewLoc, camera.getViewMatrix)
        ...
        shader.setMatrix4f(modelLoc, new Matrix4f()) // Pass identity matrix for now
