...
class Glfw(lookup: SymbolLookup, linker: Linker) {
  ...
  private val glfwSwapIntervalHandle: MethodHandle = {
    val address = lookup.find("glfwSwapInterval").get
      .orElseThrow(() => new RuntimeException("GLFW Function 'glfwSwapInterval' not found!"))

      // Returns void, accepts a single Java int
