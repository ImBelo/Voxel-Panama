...
object Window {
  ...
  def start(width: Int, height: Int, title: String): Unit = {
    ...
    withArena(Confined) { arena =>
      ...
      val posPointer: Long = MemorySegment.ofAddress(0L).address()
      // --- POSITION ATTRIBUTE ---
      // 5th param MUST be 'stride'. If it is 0 or 3*Float.BYTES, your mesh will deform completely!
      gl.vertexAttribPointer(
        0,                          // index
        3,                          // size (x, y, z)
        GL_FLOAT,                   // type
        0.toByte,                   // normalized (0.toByte means GL_FALSE)
        stride,                     // stride (24 bytes)
        posPointer                          // pointer offset (0L as a Long)
      )
      gl.enableVertexAttribArray(0)

      // --- NORMAL ATTRIBUTE ---
      // 6th param is the offset in bytes where normals start (after 3 position floats)
      // val normalOffsetBytes: Long = 3L * java.lang.Float.BYTES
      val normalOffsetBytes: Long = 3L * java.lang.Float.BYTES
      val normalPointer: Long = MemorySegment.ofAddress(normalOffsetBytes).address()
      gl.vertexAttribPointer(
        1,                          // index
        3,                          // size (nx, ny, nz)
        GL_FLOAT,                   // type
