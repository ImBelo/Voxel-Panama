...
val projectionMatrix = new org.joml.Matrix4f()


      val widthArr = Array(0)  // Temporary containers for GLFW window dimensions
      val heightArr = Array(0)
      // --- RENDER LOOP ---
      ...
      while (!shouldClose) {
        ...
        val aspectRatio = StructuredTaskScope.ShutdownOnFailure().use { _ => 
          // Create an arena that is confined to this frame block
          Arena.ofConfined().use { frameArena =>
            // Allocate 4 bytes for an integer (width and height pointers)
            val pWidth  = frameArena.allocate(ValueLayout.JAVA_INT)
            val pHeight = frameArena.allocate(ValueLayout.JAVA_INT)

            // 3. Pass the MemorySegments directly to your Panama GLFW binding
            glfw.getFramebufferSize(windowPtr, pWidth, pHeight)

            // 4. Extract the integers out of raw memory
            val width  = pWidth.get(ValueLayout.JAVA_INT, 0).toFloat
            val height = pHeight.get(ValueLayout.JAVA_INT, 0).toFloat

            if (height > 0) width / height else 1.0f
          }
        }

        // 5. Compute and upload the projection matrix
        ...
        camera.getProjectionMatrix(projectionMatrix) // Assuming you have an aspect ratio calculator here
