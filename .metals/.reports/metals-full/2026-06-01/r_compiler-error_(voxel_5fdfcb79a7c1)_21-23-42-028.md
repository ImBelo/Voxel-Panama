...
text:
```scala
class InputHandler(window: GlfwWindow, camera: Camera, mouseSensitivity: Float = 0.05f)(using arena: Arena) {
  val mouseLook = new MouseLook(camera, mouseSensitivity)
  mouseLook.register(window.handle, glfw, arena) // assuming register sets callbacks
