...
class GlfwWindow(glfw: Glfw, title: String, arena: Arena) {
  private val windowTitle = arena.allocateFrom(title) // helper to copy string + null terminator
  val handle: MemorySegment = glfw.createPrimaryWindow(windowTitle)
  ...
  def disableCursor(): Unit =
    glfw.setInputMode(handle, glfwGLFW_CURSOR, glfw.GLFW_CURSOR_DISABLED)
